...
int main(void) {
    ...
    while (1) {
        uint8_t leds = countOnes(GPIO_ReadValue(GPIO_PORT_0) & PORT0_AV_MASK);    // Counts high pins on Port 0.
...
void configGPIO(void) {
    ...
    PINSEL_ConfigMultiplePins(&pinCfg, PORT0_AV_MASK);

    pinCfg.portNum = PINSEL_PORT_2;
    PINSEL_ConfigMultiplePins(&pinCfg, LEDS_MASK);

    GPIO_SetDir(0, PORT0_AV_MASK, GPIO_INPUT);  // Sets P0.0-P0.31 as input.
    GPIO_SetDir(2, LEDS_MASK, GPIO_OUTPUT);      // Sets P2.0-P2.4 as output.

    GPIO_ClearPins(GPIO_PORT_2, LEDS_MASK);  // Clears the LEDs at startup.
}
