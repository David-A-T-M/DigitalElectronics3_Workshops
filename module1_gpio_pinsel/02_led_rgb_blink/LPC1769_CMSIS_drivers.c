...
int main(void) {
    ...
    while (1) {
        GPIO_ClearPins(GPIO_PORT_0, RED_BIT);    // Turn red LED on.
        delay();

        GPIO_SetPins(GPIO_PORT_0, RED_BIT);    // Turn red LED off.
        delay();
    }
    return 0;
}
...
void configGPIO(void) {
    ...
    GPIO_SetPins(GPIO_PORT_0, RED_BIT);    // Force red LED off initially.
}
